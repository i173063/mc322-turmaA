...
public class Seguradora {
	...
	//arquivos
	private ArquivoClientePJ clientepj;
	private ArquivoClientePF clientepf;
	
	//Construtor
	public Seguradora(String cnpj, String nome, String telefone, String email, String endereco,
			 ArrayList<Cliente> listaClientes,  ArrayList<Seguro> listaSeguros, ArquivoClientePJ clientepj,
			 ArquivoClientePF clientepf) {
		this.cnpj = cnpj;
		this.nome = nome;
	...
	//lerDados()
	public void lerDadosClientePJ(int qtdeDados) {
		int i = 0;
		while(i != qtdeDados) {
			String dado = this.clientepj.lerArquivo(i);
			System.out.printf("Cliente %d: %s", i, dado);
			i++;
		}
	}
	public void lerDadosClientePF(int qtdeDados) {
		int i = 0;
		while(i != qtdeDados) {
			String dado = this.clientepf.lerArquivo(i);
			System.out.printf("Cliente %d: %s", i, dado);
			i++;
		}
	}
	...
	//gravarDados()
	public void gravarDadosClientePJ(ClientePJ clientepj) {
		boolean dado = this.clientepj.gravarArquivo(clientepj);
		if(dado == true) {
			System.out.printf("Cliente adicionado");
		}
	}
	public void gravarDadosClientePF(ClientePF clientepf) {
		boolean dado = this.clientepf.gravarArquivo(clientepf);
		if(dado == true) {
			System.out.printf("Cliente adicionado");
		}
	}
